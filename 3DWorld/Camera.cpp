...
void Camera::Update()
{
	...
	{
		fRotX -= Input::GetInstance()->GetDeltaMove().x / 100;
		fRotY -= Input::GetInstance()->GetDeltaMove().y / 100;
	}
	...
	if (Input::GetInstance()->GetButtonDown() == true)
		D3DXVec3TransformCoord(&EyePt, &EyePt, &matRot); //눈 위치 계산
...
void Camera::SetCameraThirdPerson(GameObject * target)
{
	SetTarget(target);
}

void Camera::SetCameraFreeView()
{
	CameraFreeView = true;
}
